...
sum(is.na(rick_test$crit2))

View(rick_test)
```

After implementing the 2 first criteria, we can see that there is a problem for the availability of the data. This means that if we filter for different categories, there might not be data on every movie. This can be seen in the following plot: PLOTPLOTPLOT. 
